...
def humanmade_object_pattern(data,types):
    ...
    hmo.member_of                = collection_pattern(data["member_of"][0], types)
    ...
    for id_data in data["identified_by"]["identifiers"]:
        hmo.identified_by        = identifier_pattern(id_data, types)
    ...
    digital_object.access_point = DigitalObject(ident=id_digital_object,label='Image in full resolution')
    digital_object.classified_as = Type(id="http://vocab.getty.edu/aat/300215302",label="Digital Image")

    if "iiif_image_api" in data["representation"][0]:
        iiif_image_api = data["representation"][0]["iiif_image_api"]["id"]
        digital_object.digitally_available_via  = digital_service_pattern(data, types, iiif_image_api)
...
def digital_object_pattern(data,types):
    ...
    if "iiif_image_api" in data:
        digital_object.digitally_available_via  = digital_service_pattern(data, types, data["iiif_image_api"])
...
def type_pattern(id, types):
    ...
    type.classified_as = Type(ident="http://vocab.getty.edu/aat/300435443", label="Type of Work")
...
def negative_pattern(data, types):
    ...
    if "width" in data and data["width"] != "":
        ...
        width.unit = MeasurementUnit(id="http://vocab.getty.edu/aat/300379098", label="Centimetres")
        hmo.dimension = width
    # height
    ...
    if "height" in data and data["height"] != "":
        ...
        height.unit = MeasurementUnit(label="Centimetres", id="http://vocab.getty.edu/aat/300379098")
